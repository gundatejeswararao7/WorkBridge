...
export const ProfileCard: React.FC<ProfileCardProps> = ({ profile, onQuickView }) => {
  ...
  const getWorkMode = (): { mode: 'Remote' | 'Hybrid' | 'In-Office'; icon: typeof Laptop; color: string } => {
    ...
    // Default by category: Tech -> Remote, Non-Tech -> In-Office, Both -> Hybrid
      ...
      <div>
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold text-slate-500 truncate">
                {profile.profession || 'Skilled Member'}
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3 text-xs">
          ...
          <span className="inline-flex items-center gap-1 text-slate-500 bg-slate-50 px-2 py-0.5 rounded-full border border-slate-100">
            ...
            <span className="truncate max-w-[130px]">{profile.location || 'Global'}</span>
        ...
        <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed min-h-[32px] mb-3">
          {profile.about || profile.work_can_provide || 'Skilled professional ready to collaborate and deliver quality work on WorkBridge.'}
        </p>
