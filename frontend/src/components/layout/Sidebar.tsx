...
export const Sidebar: React.FC = () => {
  ...
  return (
    <>
      {/* ================= DESKTOP IN-FLOW COLLAPSIBLE SIDEBAR ================= */}
      {/* Uses sticky positioning in flex flow so expanding width naturally PUSHES main content without overlap */}
      <aside
        className="hidden md:flex sticky top-0 h-screen shrink-0 z-30 bg-white border-r border-slate-200/90 shadow-xs flex-col justify-between transition-[width] duration-300 ease-in-out w-[68px] hover:w-[240px] group select-none overflow-hidden"
      >
        {/* Top: Brand Logo */}
        <div>
          <div className="h-16 flex items-center px-3.5 border-b border-slate-100 overflow-hidden">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Label smoothly revealed when sidebar expands on hover */}
                  <span className="opacity-0 group-hover:opacity-100 max-w-0 group-hover:max-w-xs transition-all duration-300 whitespace-nowrap overflow-hidden text-sm font-semibold pl-1 pr-3">
                    {item.name}
                  </span>
        ...
        <div className="p-2.5 border-t border-slate-100 bg-slate-50/50">
          <Link
            to="/profile"
            className="flex items-center h-12 rounded-xl p-1 hover:bg-white hover:shadow-xs border border-transparent hover:border-slate-200/80 transition-all duration-200 overflow-hidden"
          >
            ...
            <div className="opacity-0 group-hover:opacity-100 max-w-0 group-hover:max-w-xs transition-all duration-300 whitespace-nowrap overflow-hidden pl-2 text-left flex-1 min-w-0">
          ...
          <button
            ...
            className="mt-1 flex items-center h-10 w-full rounded-xl text-slate-500 hover:text-red-600 hover:bg-red-50/80 transition-colors duration-200 overflow-hidden cursor-pointer"
          >
            <div className="w-11 h-10 flex items-center justify-center shrink-0">
              <LogOut className="w-4 h-4" />
            </div>
            <span className="opacity-0 group-hover:opacity-100 max-w-0 group-hover:max-w-xs transition-all duration-300 whitespace-nowrap overflow-hidden text-xs font-semibold pl-1 pr-3">
              Sign Out
            </span>
          </button>
        </div>
      ...
      <div className="md:hidden sticky top-0 z-40 bg-white border-b border-slate-200 px-4 h-14 flex items-center justify-between shadow-xs">
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex">
          <div className="w-64 bg-white h-full p-4 flex flex-col justify-between shadow-2xl">
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <button
                onClick={handleSignOut}
                className="flex items-center gap-2 w-full px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
              >
                <LogOut className="w-4 h-4" />
                Sign Out
              </button>
